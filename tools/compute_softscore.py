...
def filter_answers(answers_dset, min_occurence):
    ...
    all_ans_vocab = list(occurence.keys())

    for answer in all_ans_vocab:
...
def create_ans2label(occurence, name, cache_root="../cache_bin/"):
    ...
    save_pickle(ans2label, cache_file)
    cache_file = os.path.join(cache_root, name + "_label2ans.pkl")
    save_pickle(label2ans, cache_file)
    return ans2label


def compute_target(answers_dset, ans2label, name, cache_root="../cache_bin/"):
...
if __name__ == "__main__":
    train_answer_file = "../data_bin/VQA-v2/v2_mscoco_train2014_annotations.json"
    train_answers = json.load(open(train_answer_file))["annotations"]

    val_answer_file = "../data_bin/VQA-v2/v2_mscoco_val2014_annotations.json"
    val_answers = json.load(open(val_answer_file))["annotations"]

    train_question_file = (
        "../data_bin/VQA-v2/v2_OpenEnded_mscoco_train2014_questions.json"
    )
    train_questions = json.load(open(train_question_file))["questions"]

    val_question_file = "../data_bin/VQA-v2/v2_OpenEnded_mscoco_val2014_questions.json"
